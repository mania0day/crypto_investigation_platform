"""Process ETH + BNB coin PNGs into public/coins."""
import os
...
PAIRS = [
    ("eth", "ethereum_custom_color_coin"),
    ("bnb", "Binance_Coin__BNB_Glass"),
]
...
def main():
    ...
    for name, substr in PAIRS:
        ...
        if not f:
            ...
            alt = {
                "eth": ["ethereum", "eth_or_ethereum", "Premium_Photo-eab2e59e"],
                "bnb": ["Binance", "BNB_Glass", "f73e4a9d"],
            }
            for s in alt.get(name, []):
                f = find_file(files, s)
                if f:
                    break
        if not f:
        ...
        print("saved", out.name, cleaned.size, "from", f.name[-60:])
    print("done")
